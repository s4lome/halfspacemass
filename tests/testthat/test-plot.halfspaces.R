...
# test that ggplot object is created

hs <- halfspaces(data[,1:2], rescale = FALSE)
test_plot <- plot.halfspaces(hs, data = data[,1:2], display_style = "heatmap")

test_that("plot is created",
          expect_s3_class(test_plot,
                       "ggplot")
)
